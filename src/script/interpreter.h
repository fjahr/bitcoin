...
enum class SigVersion
{
    ...
    WITNESS_V2_KEYPATH = 4, //!< Witness v2 with 32-byte program, not BIP16 P2SH-wrapped, key path spending; see BIP460
};
...
struct CISAWitness {
    //! Aggregation mode marker (CISA_MARKER_OPTOUT/HALFAGG/FULLAGG).
    uint8_t marker;
    //! SIGHASH_DEFAULT unless an explicit sighash byte is present.
    uint8_t sighash_type;
    //! BIP340 signature of an opted-out input, aggregate signature on the
    //! final input of a group, or empty for placeholders. Points into the
    //! element passed to ParseCISAWitness().
