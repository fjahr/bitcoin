...
std::optional<CISAWitness> ParseCISAWitness(std::span<const unsigned char> elem, ScriptError* serror)
{
    ...
    const uint8_t marker = elem[0];
    bool has_sighash_byte{false};

    if (marker == CISA_MARKER_OPTOUT) {
        // Opted-out inputs always carry a 64-byte BIP340 signature.
        if (elem.size() != 65 && elem.size() != 66) {
            set_error(serror, SCRIPT_ERR_WITNESS_V2_INVALID_MARKER);
            return std::nullopt;
        }
        has_sighash_byte = elem.size() == 66;
    } else if (marker == CISA_MARKER_HALFAGG) {
        // 1 or 2 bytes: aggregation placeholder. (n+1)*32+1 or (n+1)*32+2
        // bytes: final input carrying the half-aggregate signature. The exact
        // signature size for the group size n is validated at transaction level.
        if (elem.size() == 2) {
            has_sighash_byte = true;
        } else if (elem.size() != 1) {
            if (elem.size() >= 65 && (elem.size() - 1) % 32 == 0) {
                has_sighash_byte = false;
            } else if (elem.size() >= 66 && (elem.size() - 2) % 32 == 0) {
                has_sighash_byte = true;
            } else {
                set_error(serror, SCRIPT_ERR_WITNESS_V2_INVALID_MARKER);
                return std::nullopt;
    ...
    } else if (marker == CISA_MARKER_FULLAGG) {
        // 1 or 2 bytes: aggregation placeholder. 65 or 66 bytes: final input
        // carrying the 64-byte full-aggregate signature.
        if (elem.size() == 2 || elem.size() == 66) {
            has_sighash_byte = true;
        } else if (elem.size() != 1 && elem.size() != 65) {
            set_error(serror, SCRIPT_ERR_WITNESS_V2_INVALID_MARKER);
            return std::nullopt;
        }
    } else {
    ...
    return CISAWitness{marker, sighash_type, elem.subspan(has_sighash_byte ? 2 : 1)};
}

/** Validate a witness v2 keypath spend at the input level (BIP460).
 *
 *  Opted-out inputs are fully verified here. For aggregated inputs only the
 *  witness structure is validated, the aggregate signatures are verified at
 *  transaction level in VerifyCISATransaction(). */
static bool VerifyWitnessV2KeyPath(const valtype& elem, const std::vector<unsigned char>& program, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror)
{
    const auto parsed{ParseCISAWitness(elem, serror)};
    if (!parsed) return false;
    if (parsed->marker == CISA_MARKER_OPTOUT) {
        // The signature message commits to the aggregation mode.
        execdata.m_cisa_agg_mode = parsed->marker;
        // Present the signature to the checker in BIP341 form: 64 bytes for
        // SIGHASH_DEFAULT, 65 bytes with the sighash type appended otherwise.
        valtype sig{parsed->signature.begin(), parsed->signature.end()};
        if (parsed->sighash_type != SIGHASH_DEFAULT) sig.push_back(parsed->sighash_type);
        if (!checker.CheckSchnorrSignature(sig, program, SigVersion::WITNESS_V2_KEYPATH, execdata, serror)) {
            return false; // serror is set
        }
    }
    return set_success(serror);
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, script_verify_flags flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // or BIP460 witness v2: same program construction with marker-based keypath spending.
        ...
        if (stack.size() == 1) {
            ...
            if (witversion == 2) {
                return VerifyWitnessV2KeyPath(stack.front(), program, checker, execdata, serror);
            }
...
bool VerifyCISATransaction(const CTransaction& tx, const std::vector<CTxOut>& spent_outputs, script_verify_flags flags, const PrecomputedTransactionData& txdata, ScriptError* serror)
{
    ...
    //! Per-input data collected in Pass 1, needed for the signature messages in Pass 3.
    struct AggInputInfo {
    ...
    for (uint32_t i = 0; i < tx.vin.size(); i++) {
        ...
        if (!parsed) return false;
        // Opted-out inputs are verified per-input, see VerifyWitnessV2KeyPath()
        if (parsed->marker == CISA_MARKER_OPTOUT) continue;

        AggGroup& group = parsed->marker == CISA_MARKER_HALFAGG ? halfagg : fullagg;
        // No group markers are allowed after the group's final input
        ...
        if (!parsed->signature.empty()) {
            group.aggsig = parsed->signature;
            group.final_seen = true;
        }
    }

    // Pass 2: validate the aggregation group structure
    if (!halfagg.inputs.empty()) {
        if (!halfagg.final_seen) return set_error(serror, SCRIPT_ERR_CISA_GROUP_INVALID);
        if (halfagg.aggsig.size() != (halfagg.inputs.size() + 1) * 32) return set_error(serror, SCRIPT_ERR_CISA_GROUP_INVALID);
    }
    if (!fullagg.inputs.empty()) {
        if (!fullagg.final_seen) return set_error(serror, SCRIPT_ERR_CISA_GROUP_INVALID);
        if (fullagg.aggsig.size() != 64) return set_error(serror, SCRIPT_ERR_CISA_GROUP_INVALID);
    }
