...
std::optional<CISAWitness> ParseCISAWitness(std::span<const unsigned char> elem, ScriptError* serror)
{
    ...
    // Opted-out inputs carry a plain BIP341 signature instead of a marked element.
    if (elem.size() == 64 || (elem.size() == 65 && elem.back() != CISA_MARKER_HALFAGG && elem.back() != CISA_MARKER_FULLAGG)) {
        return CISAWitness{/*marker=*/std::nullopt, /*sighash_type=*/SIGHASH_DEFAULT, /*signature=*/elem};
    }
    const uint8_t marker = elem.back();
    ...
    if (has_sighash_byte) {
        sighash_type = elem[elem.size() - 2];
        if (!IsValidCISASighashType(sighash_type)) {
            set_error(serror, SCRIPT_ERR_WITNESS_V2_INVALID_SIGHASH);
            return std::nullopt;
        }
    }
    return CISAWitness{marker, sighash_type, elem.first(elem.size() - (has_sighash_byte ? 2 : 1))};
}
