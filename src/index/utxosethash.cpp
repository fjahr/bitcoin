...
bool UtxoSetHash::CurrentHash(uint256& utxo_set_hash)
{
    utxo_set_hash = currentHashInternal();
    return true;
}

bool UtxoSetHash::HashAtHeight(int height, uint256& utxo_set_hash)
{
    std::pair<uint256, DBVal> read_out;
    ...
    utxo_set_hash = read_out.second.muhash;
    return true;
}

bool UtxoSetHash::HashAtBlockhash(uint256 block_hash, uint256& utxo_set_hash)
{
    std::pair<uint256, DBVal> read_out;
    if (!m_db->Read(DBHashKey(block_hash), read_out)) {
        return false;
    }
...
bool UtxoSetHash::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    // TODO: delete stuff from db instead?
    std::pair<uint256, DBVal> value;
    value.first = pindex->GetBlockHash();
    value.second.muhash = currentHashInternal();
