...
bool CoinStatsIndex::WriteBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (pindex->nHeight > 0) {
        ...
        bool added_coinbase = false;

        // Add the new utxos created from the block
        for (size_t i = 0; i < block.vtx.size(); ++i) {
            const auto& tx = block.vtx.at(i);

            // Don't add BIP30 dublicate coinbase transactions.
            // Only dealt with here and not in ReverseBlock() since a
            // rollback this deep is nearly impossible.
            if (tx->IsCoinBase() && !added_coinbase) {
                added_coinbase = true;
            } else {
              continue;
            }
