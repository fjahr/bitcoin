...
{
    // A previous version of the index used "indexes/coinstats"
    auto v1_path = gArgs.GetDataDirNet() / "indexes" / "coinstats";
    if (fs::exists(v1_path)) {
        LogWarning("Old version of coinstatsindex found at %s. " \
            "This folder can be safely deleted unless you " \
            "plan to downgrade your node to version 29 or lower.", fs::PathToString(v1_path));
    }
    fs::path path{gArgs.GetDataDirNet() / "indexes" / "coinstats2"};
