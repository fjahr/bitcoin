...
BOOST_AUTO_TEST_CASE(cisa_witness_structure_table)
{
    ...
    for (size_t size = 1; size <= 300; size++) {
        ...
        if (size >= 2) elem[size - 2] = 0x01; // valid explicit sighash type where one is expected

        // 64-byte elements are opted-out signatures even when ending in a marker
        elem.back() = CISA_MARKER_HALFAGG;
        {
            ...
            if (size == 64) {
                ...
                BOOST_CHECK_EQUAL(parsed.has_value(), valid_halfagg.contains(size));
                if (parsed) BOOST_CHECK(parsed->marker == CISA_MARKER_HALFAGG);
            }
        }
        elem.back() = CISA_MARKER_FULLAGG;
        {
            const auto parsed{ParseCISAWitness(elem)};
            if (size == 64) {
                BOOST_REQUIRE(parsed.has_value());
                ...
                BOOST_CHECK_EQUAL(parsed.has_value(), valid_fullagg.contains(size));
                if (parsed) BOOST_CHECK(parsed->marker == CISA_MARKER_FULLAGG);
            }
        }
        elem.back() = 0x01;
        {
            const auto parsed{ParseCISAWitness(elem)};
            BOOST_CHECK_EQUAL(parsed.has_value(), size == 64 || size == 65);
            if (parsed) BOOST_CHECK(!parsed->marker);
        }
    }

    // Empty elements are invalid.
    ScriptError serror;
    ...
    for (unsigned int b = 0x00; b <= 0xff; b++) {
        ...
        BOOST_CHECK_EQUAL(ParseCISAWitness(std::vector<unsigned char>{static_cast<unsigned char>(b)}).has_value(), is_marker);
    }
    ...
    for (unsigned int b = 0x00; b <= 0xff; b++) {
        const std::vector<unsigned char> elem{static_cast<unsigned char>(b), CISA_MARKER_HALFAGG};
...
BOOST_AUTO_TEST_CASE(cisa_wallet_keypath_vectors)
{
    ...
    for (const auto& vec : tests["keyPathSpending"].getValues()) {
        ...
        for (const auto& input : vec["inputSpending"].getValues()) {
            ...
            const bool opt_out{input["given"]["marker"].isNull()};
            ...
            if (opt_out) {
                ...
                execdata.m_cisa_agg_mode = ParseMarker(input["given"]["marker"].get_str());
            ...
            // Opted-out signatures are created with all-zero aux randomness
            // in the vectors.
            if (opt_out) {
                const uint256 merkle_root; // empty: keypath tweak without script tree
                std::vector<unsigned char> witness_elem(64);
                BOOST_REQUIRE(key.SignSchnorr(sighash, witness_elem, &merkle_root, uint256{}));
