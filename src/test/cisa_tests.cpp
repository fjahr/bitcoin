...
//! Parse a marker given as "0xbb" style string.
...
{
    UniValue tests;
    tests.read(json_tests::cisa_wallet_vectors);
...
{
    // Build the sets of valid element sizes for each marker from the BIP460
    // structure table and check that the parser accepts exactly those.
    std::set<size_t> valid_optout{65, 66};
    std::set<size_t> valid_fullagg{1, 2, 65, 66};
    std::set<size_t> valid_halfagg{1, 2};
    for (size_t n = 1; (n + 1) * 32 + 2 <= 300; n++) {
        valid_halfagg.insert((n + 1) * 32 + 1);
        valid_halfagg.insert((n + 1) * 32 + 2);
    }

    for (size_t size = 1; size <= 300; size++) {
        std::vector<unsigned char> elem(size);
        if (size >= 2) elem[1] = 0x01; // valid explicit sighash type where one is expected

        elem[0] = CISA_MARKER_OPTOUT;
        BOOST_CHECK_EQUAL(ParseCISAWitness(elem).has_value(), valid_optout.contains(size));
        elem[0] = CISA_MARKER_HALFAGG;
        BOOST_CHECK_EQUAL(ParseCISAWitness(elem).has_value(), valid_halfagg.contains(size));
        elem[0] = CISA_MARKER_FULLAGG;
        BOOST_CHECK_EQUAL(ParseCISAWitness(elem).has_value(), valid_fullagg.contains(size));
    }

    // Empty elements and non-marker first bytes are invalid.
    ScriptError serror;
    BOOST_CHECK(!ParseCISAWitness({}, &serror));
    ...
    for (unsigned int b = 0x00; b <= 0xff; b++) {
        ...
        elem[0] = b;
        const bool is_marker{b == CISA_MARKER_OPTOUT || b == CISA_MARKER_HALFAGG || b == CISA_MARKER_FULLAGG};
        BOOST_CHECK_EQUAL(ParseCISAWitness(elem).has_value(), is_marker);
    }
    ...
    for (unsigned int b = 0x00; b <= 0xff; b++) {
        const std::vector<unsigned char> elem{CISA_MARKER_HALFAGG, static_cast<unsigned char>(b)};
        ...
        if (!parsed) BOOST_CHECK_EQUAL(serror, SCRIPT_ERR_WITNESS_V2_INVALID_SIGHASH);
    }

    // Parsed fields point at the right pieces of the element.
    std::vector<unsigned char> elem(98);
    elem[0] = CISA_MARKER_HALFAGG;
    elem[1] = 0x83;
    const auto parsed{ParseCISAWitness(elem)};
    BOOST_REQUIRE(parsed.has_value());
    BOOST_CHECK_EQUAL(parsed->marker, CISA_MARKER_HALFAGG);
    BOOST_CHECK_EQUAL(parsed->sighash_type, 0x83);
    BOOST_CHECK_EQUAL(parsed->signature.size(), 96);
}
...
BOOST_AUTO_TEST_CASE(cisa_wallet_keypath_vectors)
{
    ...
    for (const auto& vec : tests["keyPathSpending"].getValues()) {
        ...
        for (const auto& input : vec["inputSpending"].getValues()) {
            ...
            const uint8_t marker{ParseMarker(input["given"]["marker"].get_str())};
            ...
            // Compute and check the BIP460 signature message.
            ...
            // Opted-out inputs carry a plain BIP340 signature over the
            // message, created with all-zero aux randomness in the vectors.
            if (marker == CISA_MARKER_OPTOUT) {
                ...
                std::vector<unsigned char> witness_elem{marker};
                if (hashtype != SIGHASH_DEFAULT) witness_elem.push_back(hashtype);
                witness_elem.insert(witness_elem.end(), sig.begin(), sig.end());
                BOOST_CHECK_EQUAL(HexStr(witness_elem), input["expected"]["witness"][0].get_str());
            }
        }
...
{
    UniValue tests;
    tests.read(json_tests::cisa_consensus_vectors);
