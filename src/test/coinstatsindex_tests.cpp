...
BOOST_FIXTURE_TEST_CASE(coinstatsindex_initial_sync, TestChain100Setup)
{
    ...
    const CBlockIndex* block_index;
    {
        ...
        // Coin Stats should not be found before it is started.
        BOOST_CHECK(!coin_stats_index.LookupStats(block_index, coin_stats));
    }

    // BlockUntilSyncedToCurrentChain should return false before coin_stats_index is started.
    BOOST_CHECK(!coin_stats_index.BlockUntilSyncedToCurrentChain());

    coin_stats_index.Start();

    // Allow the Coin Stats Index to catch up with the block index.
    ...
    // Check that Coin Stats Index works for genesis block.
    ...
    // Check that Coin Stats Index updates with new blocks.
