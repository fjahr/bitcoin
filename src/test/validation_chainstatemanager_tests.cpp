...
auto NoMaleation = [](CAutoFile& file, SnapshotMetadata& meta){};

template<typename F = decltype(NoMaleation)>
static bool
CreateAndActivateUTXOSnapshot(NodeContext& node, const fs::path root, F maleation = NoMaleation)
{
    ...
    maleation(auto_infile, metadata);
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_activate_snapshot, TestChain100DeterministicSetup)
{
    ...
    // Should not load maleated Snapshots
    BOOST_REQUIRE(!CreateAndActivateUTXOSnapshot(m_node, m_path_root, [](CAutoFile& auto_infile, SnapshotMetadata& metadata){
