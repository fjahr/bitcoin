...
/**
 * Class for managing case-insensitive HTTP header storage.
 * TODO: This is mostly a simplified copy of HTTPHeaders from #32061 and the
 * question is if it would be better to share that code somehow.
 */
...
public:
    std::optional<std::string> Find(const std::string& key) const
    {
        ...
        const auto it = m_map.find(lower_key);
    ...
    void Write(const std::string& key, const std::string& value)
    {
        ...
        const auto existing_value = Find(lower_key);
        if (existing_value) {
            m_map[lower_key] = existing_value.value() + ", " + value;
        } else {
    ...
    bool Read(LineReader& reader)
    {
        do {
            ...
            // An empty line indicates end of the headers section
...
HTTPReply HttpClient::ReadResponse(Sock& sock)
{
    ...
    while (!headers_complete) {
        ...
        ssize_t nrecv = sock.Recv(recv_buf, sizeof(recv_buf), 0);
    ...
    if (status_str.size() < 12 || status_str.substr(0, 5) != "HTTP/") {
    ...
    if (chunked) {
        ...
        while (true) {
            ...
            if (line_end != std::string::npos) {
                ...
                size_t chunk_size{0};
                auto [p, ec] = std::from_chars(size_str.data(), size_str.data() + size_str.size(), chunk_size, 16);
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (timeout > 0) {
        ...
        // Use a very long timeout for "indefinite"
        timeout_duration = std::chrono::seconds(5 * 365 * 24 * 60 * 60); // ~5 years
    }
