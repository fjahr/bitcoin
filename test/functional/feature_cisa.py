...
class CISATest(BitcoinTestFramework):
    ...
    def sign_optout(self, tx, spent, idx, utxo, hash_type=0, annex=None):
        ...
        sighash = cisa_signature_hash(tx, spent, idx, hash_type, CISA_MARKER_OPTOUT, annex=annex)
        sig = sign_schnorr(utxo.privkey, sighash)
        stack = [cisa_witness_element(CISA_MARKER_OPTOUT, hash_type, sig)]
        if annex is not None:
            stack.append(annex)
        tx.wit.vtxinwit[idx].scriptWitness.stack = stack

    def set_group_witness(self, tx, members, marker, aggsig, annexes):
        """Set the witnesses of an aggregation group: placeholders for all
        members, the aggregate signature on the last one."""
        for i, (idx, _, hash_type) in enumerate(members):
            sig = aggsig if i == len(members) - 1 else b""
            stack = [cisa_witness_element(marker, hash_type, sig)]
            if idx in annexes:
    ...
    def sign_halfagg_group(self, tx, spent, members, annexes=None):
        ...
        self.set_group_witness(tx, members, CISA_MARKER_HALFAGG, aggsig, annexes)
        return aggsig
    ...
    def sign_fullagg_group(self, tx, spent, members):
        ...
        self.set_group_witness(tx, members, CISA_MARKER_FULLAGG, aggsig, annexes={})
        return aggsig
    ...
    def test_optout_keypath_spend(self):
        ...
        sighash = cisa_signature_hash(tx, spent, 0, 0, CISA_MARKER_OPTOUT)
        sig = sign_schnorr(utxo.privkey, sighash)
        tx.wit.vtxinwit[0].scriptWitness.stack = [bytes([CISA_MARKER_OPTOUT, 0x00]) + sig]
    ...
    def test_cross_mode_protection(self):
        ...
        # Sign two inputs with opt-out flavored messages and fold the plain
        # signatures into a half-aggregation group, emulating a third party
        # that rewrites the markers. The signature message commits to the
        # aggregation mode, so this must fail.
        utxos = [self.fund_v2() for _ in range(2)]
        tx, spent = self.build_spend(utxos)
        pms = []
        for idx, utxo in enumerate(utxos):
            msg_optout = cisa_signature_hash(tx, spent, idx, 0, CISA_MARKER_OPTOUT)
        ...
        aggsig = halfagg_aggregate(pms)
        tx.wit.vtxinwit[0].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG)]
        tx.wit.vtxinwit[1].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG, 0, aggsig)]
        ...
        # in an opted-out witness.
        utxo = self.fund_v2()
        tx, spent = self.build_spend([utxo])
        msg_halfagg = cisa_signature_hash(tx, spent, 0, 0, CISA_MARKER_HALFAGG)
        sig = sign_schnorr(utxo.privkey, msg_halfagg)
        tx.wit.vtxinwit[0].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_OPTOUT, 0, sig)]
    ...
    def test_invalid_structures(self):
        ...
        sighash = cisa_signature_hash(tx, spent, 0, 0, CISA_MARKER_OPTOUT)
        sig = sign_schnorr(utxo.privkey, sighash)
        tx.wit.vtxinwit[0].scriptWitness.stack = [bytes([0xbe]) + sig]
        self.assert_reject(tx, "Invalid witness v2 aggregation marker")

        # Opt-out marker without a signature
        tx, spent = self.build_spend([utxo])
        tx.wit.vtxinwit[0].scriptWitness.stack = [bytes([CISA_MARKER_OPTOUT])]
        self.assert_reject(tx, "Invalid witness v2 aggregation marker")

        # Aggregation markers without a final input
        utxos = [self.fund_v2() for _ in range(2)]
        tx, spent = self.build_spend(utxos)
        tx.wit.vtxinwit[0].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG)]
        tx.wit.vtxinwit[1].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG)]
        self.assert_reject(tx, "Invalid CISA aggregation group structure")

        # Marker after the group's final input
        utxos = [self.fund_v2() for _ in range(2)]
        tx, spent = self.build_spend(utxos)
        self.sign_halfagg_group(tx, spent, [(1, utxos[1], 0), (0, utxos[0], 0)])  # final on input 0
        self.assert_reject(tx, "Invalid CISA aggregation group structure")

        # Aggregate signature size does not match the group size
        utxos = [self.fund_v2() for _ in range(2)]
        tx, spent = self.build_spend(utxos)
        aggsig = self.sign_halfagg_group(tx, spent, [(0, utxos[0], 0), (1, utxos[1], 0)])
        oversized = bytes(32) + aggsig  # 128 bytes for a two-member group
        tx.wit.vtxinwit[1].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG, 0, oversized)]
        self.assert_reject(tx, "Invalid CISA aggregation group structure")
        ...
        tx, spent = self.build_spend(utxos)
        aggsig = self.sign_halfagg_group(tx, spent, [(0, utxos[0], 0), (1, utxos[1], 0)])
        tx.wit.vtxinwit[0].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG, 0, aggsig)]
        self.assert_reject(tx, "Invalid CISA aggregation group structure")

        # Wrong aggregate signature (bit flipped)
        utxos = [self.fund_v2() for _ in range(2)]
        tx, spent = self.build_spend(utxos)
        aggsig = self.sign_halfagg_group(tx, spent, [(0, utxos[0], 0), (1, utxos[1], 0)])
        bad = bytearray(aggsig)
        bad[-1] ^= 0x01
        tx.wit.vtxinwit[1].scriptWitness.stack = [cisa_witness_element(CISA_MARKER_HALFAGG, 0, bytes(bad))]
        self.assert_reject(tx, "CISA aggregate signature verification failed")
