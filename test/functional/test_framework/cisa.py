...
def cisa_signature_hash(tx, spent_utxos, input_index, hash_type, agg_mode, annex=None):
    """Compute the BIP460 witness v2 keypath signature message:
...
def cisa_witness_element(marker, hash_type=0, sig=b""):
    """Construct a witness v2 keypath witness element. The sighash byte is
    only present for non-default sighash types."""
    elem = bytes([marker])
    if hash_type != 0:
