...
def cisa_witness_element(marker, hash_type=0, sig=b""):
    """Construct an aggregated witness v2 keypath witness element with the
    marker last, preceded by a sighash byte for non-default sighash types."""
    elem = sig
    if hash_type != 0:
        elem += bytes([hash_type])
    return elem + bytes([marker])
