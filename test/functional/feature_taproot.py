...
def spender_legacy(spenders, key, pubkey, comment, hashtype, segwit, p2sh):
    ...
    def fn(t, i, u, v):
        ...
        sig = key.sign_ecdsa(sig_hash) + bytes([hashtype])
        if segwit:
...
class TaprootTest(BitcoinTestFramework):
    ...
    def test_spenders(self, spenders, input_counts):
        ...
        while utxos:
            ...
            # Pick a variable number of UTXOs to construct transaction inputs
            ...
            sigops_weight = sum(input_utxos[i].spender.sigops_weight for i in range(inputs))
            ...
            for fail_input in range(inputs + 1):
                ...
                self.block_submit(self.nodes[0], [tx], msg, witness=True, accept=fail_input == inputs, cb_pubkey=cb_pubkey, fees=fee, sigops_weight=sigops_weight, err_msg=expected_fail_msg)

    def build_spenders(self):
        VALID_SIGHASHES_ECDSA = [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE, SIGHASH_ANYONECANPAY + SIGHASH_ALL,
        ...
        for annex in [None, bytes([ANNEX_TAG]) + random_bytes(random.randrange(0, 250))]:
            ...
            hashtype = random.choice(VALID_SIGHASHES_TAPROOT)
            scripts = [
            ...
            # In the next test, we verify the sigops ratio limit implementation.
            # We build scripts that pop off a D-byte dummy from the stack, and then execute either N or N+1
            # (based on a flag input) successful OP_CHECKSIGVERIFYs, such that the ratio limit passes for N, but
            # fails for N+1. In order to do so, we must predict the exact size of the witness that will be used.
            # The witness consists of:
            # - CompactSize encoding of the number of witness elements (5 or 6): 1 WU
            # - The 3 witness elements that serve as input:
            #   - The signature: 64 or 65 WU (depending on hashtype != 0) + 1 WU size
            #   - The flag input: 1 WU + 1 WU size
            #   - The dummy: D WU + 1 or 2 WU size (depending on D > 253 or not)
            # - The script itself: (39+2*N) WU + 1 WU size
            # - The control block: 65 WU (incl. 1 Merkle step) + 1 WU size
            # - Optionally, annex of size A WU + 1 WU size
            # This means an overall budget of (175 + 2*N + hashtype!=0 + D + (D > 253) + A + annex? + 50) WU.
            # To permit N sigops, this requires 50*N <= 225 + 2*N + hashtype!=0 + D + (D > 253) + A + annex?, or
            # N <= (225 + hashtype!=0 + D + (D > 253) + A + annex?) / 48. We do this twice, once for D <= 253, and
            # once for D > 253.
            dummy_small = random_bytes(random.randrange(0, 253))
            dummy_large = random_bytes(random.randrange(253, 521))
            annex_size = 0 if annex is None else len(annex) + 1
            n_small = (225 + (hashtype != 0) + len(dummy_small) + annex_size) // 48
            n_large = (226 + (hashtype != 0) + len(dummy_large) + annex_size) // 48
            scripts = [
                # 0) Script with variable number of duplicated signature checks (designed for a push of dummy_small)
                CScript([OP_DROP, pub2, OP_SWAP, OP_1SUB, OP_IF, OP_2DUP, OP_CHECKSIGVERIFY, OP_ENDIF] + [OP_2DUP, OP_CHECKSIGVERIFY] * (n_small - 1) + [OP_CHECKSIG]),
                # 1) Script with variable number of duplicated signature checks (designed for a push of dummy_large)
                CScript([OP_DROP, pub2, OP_SWAP, OP_1SUB, OP_IF, OP_2DUP, OP_CHECKSIGVERIFY, OP_ENDIF] + [OP_2DUP, OP_CHECKSIGVERIFY] * (n_large - 1) + [OP_CHECKSIG])
            ]
            info = taproot_construct(pub1, scripts)
            # Test that the (witsize+50 >= 50*(1+sigchecks)) rule is enforced (but only for executed checksigs)
            spender_two_paths(spenders, info, "tapscript/sigopratiosmall", standard=no_annex and len(dummy_small) <= 80, success={"key": sec2, "hashtype": hashtype, "annex": annex, "script": scripts[0], "suffix": [bytes([1]), dummy_small]}, failure={"key": sec2, "hashtype": hashtype, "annex": annex, "script": scripts[0], "suffix": [bytes([2]), dummy_small]}, err_msg="Too much signature validation relative to witness weight")
            spender_two_paths(spenders, info, "tapscript/sigopratiolarge", standard=False, success={"key": sec2, "hashtype": hashtype, "annex": annex, "script": scripts[1], "suffix": [bytes([1]), dummy_large]}, failure={"key": sec2, "hashtype": hashtype, "annex": annex, "script": scripts[1], "suffix": [bytes([2]), dummy_large]}, err_msg="Too much signature validation relative to witness weight")
            ...
            info = taproot_construct(pub1, scripts)
            spender_sighash_mutation(spenders, info, "alwaysvalid/pk", key=sec1, hashtype=random.choice(VALID_SIGHASHES_TAPROOT), annex=annex, standard=no_annex)
        ...
        # Also add a few legacy spends into the mix, so that transactions which combine taproot and
        # pre-taproot spends get tested too.
        eckey = ECKey()
        eckey.set(generate_privkey(), True)
        pubkey = eckey.get_pubkey().get_bytes()
        for hashtype in VALID_SIGHASHES_ECDSA:
            spender_legacy(spenders, eckey, pubkey, "legacy/p2pkh", hashtype, False, False)
            spender_legacy(spenders, eckey, pubkey, "legacy/p2sh_p2pk", hashtype, False, True)
            spender_legacy(spenders, eckey, pubkey, "legacy/p2wpkh", hashtype, True, False)
            spender_legacy(spenders, eckey, pubkey, "legacy/p2sh_p2wpk", hashtype, True, True)
