...
This tool assumes a build directory named `build` just as suggested most
documentation examples.To use it with a different build dir set `BUILDDIR`.
For example:

```bash
BUILDDIR=$PWD/my-build contrib/devtools/gen-manpages.py
```
...
```bash
BUILDDIR=$PWD/my-build contrib/devtools/gen-bitcoin-conf.sh
```
